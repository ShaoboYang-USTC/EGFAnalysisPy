...
def plot_test(fig1, prob_G, curve_G, fig2, prob_C, curve_C, name, 
    ...
    plt.pcolor(x, y, prob_G, shading='auto', cmap='jet', vmin=0, vmax=1.05)
    plt.colorbar()
    plt.xlabel('Period (s)',fontsize=fontsize)
    plt.ylabel('Group Velocity (km/s)',fontsize=fontsize)
    plt.title('Group probability',fontsize=fontsize)
    plt.tick_params(labelsize=15)

    plt.subplot(222)       # after correction
    image = fig1
    ...
    plt.subplot(223)
    plt.pcolor(x, y, prob_C, shading='auto', cmap='jet', vmin=0, vmax=1.05)
    plt.colorbar()
    plt.xlabel('Period (s)',fontsize=fontsize)
    plt.ylabel('Phase Velocity (km/s)',fontsize=fontsize)
    plt.title('Phase probability',fontsize=fontsize)
    plt.tick_params(labelsize=15)

    plt.subplot(224)         # after correction
    image = fig2
